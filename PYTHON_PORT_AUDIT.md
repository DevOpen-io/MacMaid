...
Installation and normal operation are user-local and reject root execution. DeepClean never restarts itself with `sudo` and application removal never elevates. The separately reviewed advanced Spotlight rebuild may invoke one native macOS authorization dialog scoped to that fixed maintenance operation; cancellation fails closed.
